...
public class AbstractSpentCoin<G extends Group<G>, F extends FiniteField<F>, I, T, H extends SignatureHashFunction<G, F>, H0 extends PaymentHashFunction<G, F, I, T>>
	...
	public AbstractSpentCoin(UnspentCoin<G, F, I, T, H, H0> basedOnCoin,
		...
		// Generated by shop
		FiniteField.Element<F> d = setup.getPaymentHash().hash(blindedIdentity,
				commitment, bearer.getIdentity(), time);

		// Generated by account holder
		spendingCommitments = (FiniteField.Element<F>[]) Array.newInstance(
		...
		spendingCommitments[1] = d.multiply(basedOnCoin.getBlindingFactor())
				.add(basedOnCoin.getPayerWitness()[1]);

		// Tested by shop
		ExampleElement left = (ExampleElement) setup.getGenerators()[1]
				.exponentiate(spendingCommitments[0]).multiply(
						setup.getGenerators()[2]
								.exponentiate(spendingCommitments[1]));
		ExampleElement right = (ExampleElement) blindedIdentity.exponentiate(d)
				.multiply(commitment);
